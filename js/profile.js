document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    container.innerHTML = `
      <div class="space-y-6 slide-up">
        <div class="flex flex-col gap-1 border-b border-gray-100 pb-4">
          ...
          <span class="text-lg font-medium">${data['이름']}</span>
        </div>
        <div class="flex flex-col gap-1 border-b border-gray-100 pb-4">
          <span class="text-xs font-bold text-gray-400">아이디 (사번)</span>
          <span class="text-lg font-medium">${data['아이디']}</span>
        </div>
        <div class="flex flex-col gap-1 border-b border-gray-100 pb-4">
          <span class="text-xs font-bold text-gray-400">전화번호</span>
          <span class="text-lg font-medium">${data['전화번호']}</span>
        </div>
    ...
    container.innerHTML = '<div class="text-center text-red-500">정보를 불러오는데 실패했습니다.</div>';
  }
  ...
  withdrawBtn.addEventListener('click', () => {
    const modalHtml = `
      <div id="pw-modal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 fade-in">
        <div class="bg-container w-[320px] rounded-xl shadow-soft p-6 slide-up">
          ...
          <input type="password" id="withdrawPw" class="w-full h-10 px-3 border border-gray-300 rounded mb-6 outline-none focus:border-red-500" placeholder="비밀번호">
          <div class="flex gap-2">
            <button id="pw-cancel" class="flex-1 py-2.5 rounded-lg bg-gray-100 font-medium hover:bg-gray-200 transition">취소</button>
            <button id="pw-confirm" class="flex-1 py-2.5 rounded-lg bg-red-500 text-white font-medium hover:bg-red-600 transition">탈퇴</button>
          </div>
        </div>
      </div>
    `;
    ...
    document.getElementById('pw-cancel').onclick = () => document.getElementById('pw-modal').remove();
    ...
    document.getElementById('pw-confirm').onclick = async () => {
      ...
      try {
        await fetchAPI('deleteUser', { userId: user.id, password: pw }, 'POST');
