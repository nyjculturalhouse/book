...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    currentContainer.innerHTML = `<div class="text-center text-red-500 py-8">${err.message || '데이터를 불러오는데 실패했습니다.'}</div>`;
  }
});
    ...
    <div class="bg-container rounded-xl p-4 shadow-soft flex gap-4 items-center slide-up">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 mb-1">
          <h3 class="text-md font-bold truncate">${UI.escapeHtml(item['제목'])}</h3>
          ${statusBadge}
        </div>
      ...
      <button onclick="returnBook('${safeIsbnForJs}', '${safeTitleForJs}')" class="px-4 py-2 bg-primary text-white rounded-lg text-sm font-bold hover:bg-primary-hover transition-colors shrink-0">
        반납하기
      </button>
    </div>
  `;
  ...
  return `
    <div class="bg-container rounded-xl p-4 shadow-soft flex flex-col gap-2 slide-up">
      <div class="flex justify-between items-start">
        <h3 class="text-md font-bold text-gray-800">${UI.escapeHtml(item['제목'])}</h3>
        ${isOverdue
          ? `<span class="text-xs font-bold text-red-500 bg-red-50 px-2 py-1 rounded">연체 (${item['연체일수']}일)</span>`
          : `<span class="text-xs font-bold text-green-600 bg-green-50 px-2 py-1 rounded">정상반납</span>`}
      </div>
